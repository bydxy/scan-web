...
## ADR-009 全屏识别而非取景框内识别

**决策**：识别范围 = 屏幕上实际可见的整个相机画面（cover 裁切后可见部分），取景框四角仅作视觉引导。

**理由**：用户预期「指哪扫哪」；被 cover 裁掉的画面边缘不可见，识别它们是白付算力——所以裁剪源从中央 ROI 改为可见矩形，长边降采样阶梯（720→1080→原生）继续兜住速度。

## ADR-010 空间化标注 + App 识别跳转层

**决策**：
1. 标注不再是无大小概念的角标 pill，而是 **SVG polygon 描边直接贴合码的四角**——随码的大小伸缩、随移动移动、旋转/透视变形也能包住；内容 chip 骑在包围盒上方（顶部空间不足自动落下方）
2. 新增 `apps.ts` 解析层：原生支付 scheme 直通（wxp://、alipays://）→ 已知域名命中 App 规则表（支付宝二维码用 `saId=10000007&qrcode=` 唤起、抖音/快手/百度有官方 webview scheme，其余知名域名仅命名、跳转交给 Universal Link）→ 未知名域只显 hostname → 纯文本一键复制为主操作

**理由**：
- chip 若不跟随码的空间几何，「标注」就退化为普通列表提示；SVG polygon 是零成本获得像素级贴合的方案
- scheme 表只收录**已核实可靠**的构造器，其余硬造 scheme 会静默失败反而伤体验；无 scheme 的知名 App 用系统 UL/App Link 自动路由
- 已知 App 显示名称、未知域名显示 hostname、文本显示截断内容——信息密度与用户决策需求匹配
